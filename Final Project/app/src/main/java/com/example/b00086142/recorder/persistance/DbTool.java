package com.example.b00086142.recorder.persistance;
...
public final class DbTool {
    ...
    public int queryLastHeartRate() {
        Cursor cursor = mDb.rawQuery("select * from " + FitnessDbSchema.HeartRateTable.NAME + " order by " + FitnessDbSchema.HeartRateTable.Cols.timestamp + " desc limit 1", null);
        if (cursor.moveToNext()) {
            int anInt = cursor.getInt(cursor.getColumnIndex(FitnessDbSchema.HeartRateTable.Cols.heartRate));
            return anInt;
        }
        return 0;
    }
